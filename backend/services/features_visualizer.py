...
class FeatureEngineerVisualizer:
    ...
    # ======================================================
    # 🧠 استخراج الميزات الأساسية فقط - مُحسّن
    # ======================================================
    def extract_sequence_features(self, gesture: Dict) -> np.ndarray:
        ...
        for i, frame in enumerate(frames):
            ...
            # استخراج الإحداثيات الأساسية فقط
            x = np.array([p.get('x', 0) for p in points])
            y = np.array([p.get('y', 0) for p in points])
            pressure = np.array([p.get('pressure', 0.5) for p in points])

            # ✅ حساب السرعة فقط (إزالة التسارع لتقليل التعقيد)
            ...
            # ✅ ميزات أساسية فقط (تقليل من 21 إلى 9 ميزات)
            ...
            frame_features = [
                ...
                # الضغط (ميزة مهمة للخط العربي)
                np.mean(pressure) if len(pressure) > 0 else 0.5,
                
                # السرعة (ميزة ديناميكية مهمة)
                np.mean(velocity_magnitude) if len(velocity_magnitude) > 0 else 0.0,
                
                # معلومات الوقت وعدد النقاط
                delta_s,
                len(points),
                
                # نسبة تغطية المساحة
                (np.max(x) - np.min(x)) * (np.max(y) - np.min(y)) if len(x) > 0 and len(y) > 0 else 0.0
            ]
    ...
    # ======================================================
    # 🔄 تقسيم البيانات مع تحسين الميزات
    # ======================================================
    def split_data(self, gestures_data: List[Dict], fixed_indices=None):
        features = []  # ✅ تصحيح هنا - تعريف list منفصل
        labels = []    # ✅ تصحيح هنا - تعريف list منفصل
        ...
        print(f"🎯 Reduced features from 21 to {X.shape[2]} per timestep")
        ...
        split_info = {
            ...
            "reduced_features_count": n_features
        }
    ...
    # ======================================================
    # 📈 حفظ إحصائيات الميزات المُحسّنة
    # ======================================================
    def plot_feature_distribution(self, gestures_data: List[Dict]):
        # ✅ تقليل الميزات التي نحلل توزيعها
        features_to_save = ['x', 'y', 'pressure', 'delta_ms']
        agg = {}
