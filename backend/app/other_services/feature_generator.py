...
class StatisticalFeatureGenerator:
    """
    نسخة معدلة من StatisticalFeatureGenerator
    → تولد كل الفيتشرات المتقدمة لكل فريم
    → تنتج CSV بصيغة WIDE جاهز للتحليل
    """
    ...
    # ---------------- استخراج الفيتشر لفريم واحد ----------------
    def _extract_frame_features(self, frame: Dict, prev_frame: Dict = None,
                                prev_velocity: np.ndarray = None, cumulative_dist: float = 0.0) -> Dict:
        ...
        stroke_progress = cumulative_dist  # يمكن تطبيعها لاحقًا

        # curvature مبسطة 2D (لثلاث نقاط)
        curvature = 0.0
        if prev_frame is not None:
    ...
    # ---------------- تحويل gesture إلى مصفوفة ثابتة ----------------
    ...
    # ---------------- حفظ كل الـ gestures في CSV بصيغة WIDE ----------------
    def process_gestures(self, gestures: List[Dict], out_csv: str = "advanced_features.csv") -> str:
        rows = []
        cols = ["gesture_id", "character", "orig_frame_count"] + [
            f"t{t:03d}_{fn}" for t in range(self.max_timesteps) for fn in self.per_frame_names
        ]
        ...
        if self.verbose:
            print(f"✅ Saved CSV with advanced features → {out_csv}")
